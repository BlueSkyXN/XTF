...
class FeishuAPIClient:
    ...
    def create_field(
        ...
        url = f"https://open.feishu.cn/open-apis/bitable/v1/apps/{app_token}/tables/{table_id}/fields"
        headers = self.get_auth_headers()
        data = {"field_name": field_name, "type": field_type}  # 1=多行文本
...
class XTFSyncEngine:
    ...
    def get_field_types(self) -> Dict[str, int]:
        """获取字段类型映射"""
        try:
            existing_fields = self.api_client.list_fields(
    ...
    def ensure_fields_exist(self, df: pd.DataFrame) -> Tuple[bool, Dict[str, int]]:
        ...
        try:
            ...
            if self.config.create_missing_fields:
                ...
                if missing_fields:
                    self.logger.info(
                        f"需要创建 {len(missing_fields)} 个缺失字段: {', '.join(missing_fields)}"
                    )
                    ...
                    for field_name in missing_fields:
                        ...
                        self.logger.info(
                            f"字段 '{field_name}': {analysis['analysis']}, "
        ...
        for i in range(0, len(items), batch_size):
            ...
            try:
                ...
                if processor_func(*args, batch, **kwargs):
                    success_count += 1
                    self.logger.info(
                        f"批次 {batch_num}/{total_batches} 处理成功 ({len(batch)} 条记录)"
                    )
                else:
    ...
    def sync_full(
        ...
        # 获取现有记录并建立索引
        existing_records = self.api_client.get_all_records(
            self.config.app_token, self.config.table_id
        )
            ...
            for col_name, value in row.to_dict().items():
                if pd.notnull(value) and col_name in field_types:
                    ...
                    if field_type == 2 and isinstance(
                        ...
                        if not (
                            self._is_timestamp_string(str(value))
                            or self._is_date_string(str(value))
                        ):
                            mismatch_warnings.append(
...
class ConfigManager:
    ...
    def parse_args() -> argparse.Namespace:
        ...
        # 功能开关
        parser.add_argument(
            "--no-create-fields", action="store_true", help="不自动创建缺失字段"
        )

...
# -*- coding: utf-8 -*-
"""
XTF_Sheet - 独立电子表格同步脚本（Legacy）

模块概述：
    这是 XTF 工具的独立电子表格同步脚本，包含完整的同步功能，
    无需依赖其他模块即可运行。此脚本主要用于单文件部署场景
    或 CI/CD 自动构建的独立二进制文件。

⚠️ 注意：此为 Legacy 版本
    推荐使用模块化版本：python XTF.py --target-type sheet
    此脚本保留用于向后兼容和单文件部署需求。
...
使用方法：
    # 使用配置文件
    $ python XTF_Sheet.py --config config.yaml
    
    # 命令行指定参数
    $ python XTF_Sheet.py --file-path data.xlsx \\
        --app-id cli_xxx --app-secret xxx \\
        --spreadsheet-token xxx --sheet-id xxx
...
class RetryableAPIClient:
    ...
    def __init__(
        self, max_retries: int = 3, rate_limiter: Optional[RateLimiter] = None
    ):
...
class FeishuSheetAPIClient:
    ...
    def get_tenant_access_token(self) -> str:
        ...
        data = {"app_id": self.config.app_id, "app_secret": self.config.app_secret}
    ...
    def write_sheet_data(
        ...
        if result.get("code") != 0:
            error_msg = result.get("msg", "未知错误")
            self.logger.error(
                f"写入电子表格数据失败: 错误码 {result.get('code')}, 错误信息: {error_msg}"
            )
    ...
    def append_sheet_data(
        ...
        if result.get("code") != 0:
            error_msg = result.get("msg", "未知错误")
            self.logger.error(
                f"追加电子表格数据失败: 错误码 {result.get('code')}, 错误信息: {error_msg}"
            )
            self.logger.debug(f"API响应: {result}")
            return False
...
class XTFSheetSyncEngine:
    ...
    def get_range_string(
        self, start_row: int, start_col: str, end_row: int, end_col: str
    ) -> str:
        """生成范围字符串"""
    ...
    def get_current_sheet_data(self) -> pd.DataFrame:
        ...
        try:
            values = self.api_client.get_sheet_data(
                self.config.spreadsheet_token, range_str
            )
    ...
    def sync_full(self, df: pd.DataFrame) -> bool:
        ...
        if update_rows:
            ...
            success = self.api_client.write_sheet_data(
                self.config.spreadsheet_token, range_str, values
            )
        ...
        if new_rows and success:
            ...
            if new_values:
                ...
                range_str = self.get_range_string(
                    start_row, "A", start_row + len(new_values) - 1, end_col_letter
                )
                success = self.api_client.append_sheet_data(
                    self.config.spreadsheet_token, range_str, new_values
                )
    ...
    def sync_incremental(self, df: pd.DataFrame) -> bool:
        ...
        if not self.config.index_column:
            ...
            return self.api_client.append_sheet_data(
                self.config.spreadsheet_token, range_str, values
            )
    ...
    def sync_overwrite(self, df: pd.DataFrame) -> bool:
        ...
        if new_df_rows:
            ...
            # 先清空现有数据，然后写入新数据
            return self.api_client.write_sheet_data(
                self.config.spreadsheet_token, range_str, values
            )
        else:
            # 如果没有数据，清空表格
            return self.api_client.clear_sheet_data(
                self.config.spreadsheet_token, f"{self.config.sheet_id}!A:Z"
            )
...
class ConfigManager:
    ...
    def parse_args() -> argparse.Namespace:
        ...
        parser = argparse.ArgumentParser(
            description="XTF_Sheet - Excel To Feishu 同步工具（支持多维表格和电子表格）"
        )
        ...
        parser.add_argument(
            "--create-missing-fields", action="store_true", help="自动创建缺失字段"
        )
...
def main():
    ...
    try:
        ...
        if success:
            ...
            print(
                f"📊 同步到电子表格: https://feishu.cn/sheets/{config.spreadsheet_token}"
            )
        else:
            print("\n❌ 同步过程中出现错误，请查看日志文件")
